...
class DbHelper:
    ...
    def get_predictions(self, run_name, comparison_name, event_no):
        ...
        query = 'SELECT * FROM scalar WHERE event = {}'.format(event_no)
        event_scalars = pd.read_sql_query(query, self.con)
        query = 'SELECT * FROM sequential WHERE event = {}'.format(event_no)
        ...
        query = 'SELECT * FROM meta WHERE event = {}'.format(event_no)
